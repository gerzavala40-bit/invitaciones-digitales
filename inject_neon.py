...
path = os.path.join(demos_dir, 'despedida-neon', 'page.tsx')
with open(path, 'r', encoding='utf-8') as f:
    content = f.read()
    
# Find WhatsApp button to replace
if "https://wa.me/" in content:
    custom_form = form_template.replace("{event_id}", "despedida-neon")
    new_content = wa_btn_regex.sub(custom_form, content, count=1)
    with open(path, 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("Migrated despedida-neon")
else:
    print("Skipped despedida-neon (no WhatsApp button found)")
