...
import MSNMessageBubble from "./MSNMessageBubble";
import MSNContactList from "./MSNContactList";
import MSNStatusBar from "./MSNStatusBar";
...
export default function MSNChatWindow({ eventId, eventTitle, participant, onLogout }: MSNChatWindowProps) {
    ...
    try {
      ...
      if (data.messages && data.messages.length > 0) {
        setMessages((prev) => {
          ...
          // Play sound for new text messages from others
          const hasNewFromOthers = newMsgs.some(
            (m: ChatMessage) => m.messageType === "text" && m.sender.id !== participant.id
          );
          if (hasNewFromOthers && audioRef.current) {
  ...
  }, [eventId, activeChannel, participant.token, participant.id]);

  // Fetch participants
  const fetchParticipants = useCallback(async () => {
    try {
      const params = new URLSearchParams({ eventId, token: participant.token });
      const res = await fetch(`/api/chat/participants?${params}`);
      if (!res.ok) return;
      const data = await res.json();
  ...
  return (
    <div className={`min-h-screen bg-[#1a3a5c] flex flex-col items-center p-2 sm:p-4 ${nudgeShake ? "animate-shake" : ""}`}>
      ...
      <audio ref={audioRef} preload="auto">
        <source src="data:audio/wav;base64,UklGRnoGAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQoGAACBhYqFbF1fdH2LkY2Ff3N4fYaRk42Eg3d1fIOLj42JhH55eX+Gi46NiYaCfHt+goiLjImHg4B+fYGFiIqKiIWDgX9+gYSHiYmIhoSDgYB/gYOGh4iHhoWDgoGAgIKEBQ==" type="audio/wav" />
      </audio>
      <audio ref={nudgeAudioRef} preload="auto">
        <source src="data:audio/wav;base64,UklGRnoGAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQoGAACBhYqFbF1fdH2LkY2Ff3N4fYaRk42Eg3d1fIOLj42JhH55eX+Gi46NiYaCfHt+goiLjImHg4B+fYGFiIqKiIWDgX9+gYSHiYmIhoSDgYB/gYOGh4iHhoWDgoGAgIKEBQ==" type="audio/wav" />
      </audio>
