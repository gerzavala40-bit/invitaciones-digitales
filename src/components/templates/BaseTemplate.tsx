...
export default function BaseTemplate({ event, config }: { event: EventData; config: TemplateConfig }) {
    ...
    try {
      const res = await fetch("/api/rsvp", {
        ...
        body: JSON.stringify({
          ...
          guestCount: (parseInt(fd.get("adultos") as string) || 0) + (parseInt(fd.get("ninos") as string) || 0),
  ...
  const css = `
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .hero {
      background: ${p.heroBg};
      ${p.heroBg.includes('url') ? "text-shadow: 0 2px 8px rgba(0,0,0,0.85);" : ""}
    }
    ${p.heroBg.includes('url') ? `
      .hero h1 { text-shadow: 0 4px 16px rgba(0,0,0,0.95); }
      .hero-kicker, .hero-date, .scroll-cue { text-shadow: 0 2px 8px rgba(0,0,0,0.9); }
    ` : ""}
    ...
    .hero-kicker { font-size:0.95rem; letter-spacing:0.35em; text-transform:uppercase; color:var(--accent); font-weight:500; margin-bottom:0.8rem; }
  ...
  return (
    <>
      ...
      <div id="splash" className={entered ? "hide" : ""} onClick={handleEnter}>
        {event.slug === '15anos-clara' && (
          <video autoPlay loop muted playsInline style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', objectFit: 'cover', zIndex: -1 }}>
            <source src="/mariposas_celestes_y_blancas_c.mp4" type="video/mp4" />
          </video>
        )}
      ...
      <div id="main" className={entered ? "show" : ""}>
        <section className="section hero" id="inicio">
          <div className="wrap stagger-reveal">
            {event.photos && event.photos.length > 0
              ...
              : event.eventType === "boda" ? (
                <img src="/casamiento_icon.png" style={{ width: "110px", height: "auto", marginBottom: "20px", filter: "drop-shadow(0 2px 8px rgba(0,0,0,0.85)) brightness(1.15)", display: "inline-block" }} alt="Icono casamiento" />
              ) : (
                ...
                <form className="card rsvp-form reveal" onSubmit={handleRsvp}>
                  ...
                  <div style={{ display: "flex", gap: "1rem" }}>
                    <div style={{ flex: 1 }}>
                      ...
                      <input id="adultos" name="adultos" type="number" min="1" defaultValue={1} />
                    </div>
                    <div style={{ flex: 1 }}>
                      <label htmlFor="ninos">Niños</label>
                      <input id="ninos" name="ninos" type="number" min="0" defaultValue={0} />
                    </div>
                  </div>
        ...
        {(event.slug === '15anos-clara' || event.slug === 'boda-valentina-y-matias') && (
          <section className="section">
        ...
        {event.bankAlias && (
          <section className="section">
            <div className="wrap reveal">
              ...
              <p className="lead" style={{ marginBottom: "1.5rem" }}>Tu presencia es el mejor regalo.</p>
              {!showGifts ? (
                <button type="button" className="copy-btn" style={{ background: "transparent", border: "1px solid var(--accent)", color: "var(--accent)", padding: "10px 24px", margin: "0 auto", display: "inline-block" }} onClick={() => setShowGifts(true)}>
                  Ver datos bancarios
                </button>
              ) : (
                <div className="alias-box">
                  ...
                  <button type="button" style={{ display: "block", margin: "12px auto 0", background: "transparent", border: "none", color: "var(--text-muted)", fontSize: "11px", textDecoration: "underline", cursor: "pointer" }} onClick={() => setShowGifts(false)}>
                    Ocultar datos
                  </button>
                </div>
              )}
